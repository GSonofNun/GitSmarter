...
// Table of Contents
//
// 1. Path Containment Tests (line 33)
// 2. Pack Index Fanout Monotonicity Tests (line 204)
// 3. Commit-Graph BFS Tests (non-first-parent traversal) (line 303)
// 4. Packed-ref Namespace Collision Tests (line 502)
//
// </AUTO-GENERATED TOC>
...
// Hostile trees placing a `.git` component would overwrite repository metadata.
TEST(path_safe_rejects_dot_git_component) {
    TEST_ASSERT_FALSE(test_git_is_safe_relative_path(".git"));
    TEST_ASSERT_FALSE(test_git_is_safe_relative_path(".git/config"));
    TEST_ASSERT_FALSE(test_git_is_safe_relative_path(".git/hooks/post-checkout"));
    TEST_ASSERT_FALSE(test_git_is_safe_relative_path("foo/.git/bar"));
    TEST_ASSERT_FALSE(test_git_is_safe_relative_path("FOO/.GIT/hooks"));  // case-insensitive
    // Legitimate names that merely contain the letters "git" remain allowed
    TEST_ASSERT_TRUE(test_git_is_safe_relative_path(".gitignore"));
    TEST_ASSERT_TRUE(test_git_is_safe_relative_path(".gitattributes"));
    TEST_ASSERT_TRUE(test_git_is_safe_relative_path("foo.git/bar"));
}

// Windows device basenames (and trailing dots/spaces) must not be materialised.
TEST(path_safe_rejects_windows_reserved_and_trailing) {
    TEST_ASSERT_FALSE(test_git_is_safe_relative_path("CON"));
    TEST_ASSERT_FALSE(test_git_is_safe_relative_path("con"));
    TEST_ASSERT_FALSE(test_git_is_safe_relative_path("aux.txt"));
    TEST_ASSERT_FALSE(test_git_is_safe_relative_path("dir/NUL"));
    TEST_ASSERT_FALSE(test_git_is_safe_relative_path("COM1"));
    TEST_ASSERT_FALSE(test_git_is_safe_relative_path("lpt9.log"));
    TEST_ASSERT_FALSE(test_git_is_safe_relative_path("file.txt."));   // trailing dot
    TEST_ASSERT_FALSE(test_git_is_safe_relative_path("file.txt "));   // trailing space
    TEST_ASSERT_FALSE(test_git_is_safe_relative_path("dir./file"));   // segment trailing dot
    // Non-reserved near-misses
    TEST_ASSERT_TRUE(test_git_is_safe_relative_path("console.txt"));
    TEST_ASSERT_TRUE(test_git_is_safe_relative_path("COM10"));  // not COM1–COM9
    TEST_ASSERT_TRUE(test_git_is_safe_relative_path("null.txt"));
}
...
// make_worktree_path must refuse `.git` / reserved names even if a caller
// forgot an earlier git_is_safe_relative_path check.
TEST(make_worktree_path_rejects_dot_git_and_reserved) {
    GitRepository repo = {};
    if (!git_repo_open(&repo, TEST_REPO_PATH)) {
        TEST_SKIP("Fixture repo not found");
    }
    ...
    wchar_t out_path[Git::MAX_PATH_LEN];
    TEST_ASSERT_FALSE(test_git_make_worktree_path(&repo, ".git/config", out_path, Git::MAX_PATH_LEN));
    TEST_ASSERT_FALSE(test_git_make_worktree_path(&repo, ".GIT/hooks/x", out_path, Git::MAX_PATH_LEN));
    TEST_ASSERT_FALSE(test_git_make_worktree_path(&repo, "CON", out_path, Git::MAX_PATH_LEN));
    TEST_ASSERT_FALSE(test_git_make_worktree_path(&repo, "aux.txt", out_path, Git::MAX_PATH_LEN));
...
// Push non-FF check uses is_fast_forward(repo, remote, local): remote must be
// an ancestor of local through the full graph (both parents). First-parent-only
// walks miss second-parent ancestry after merges (FIND-039).
TEST(push_ff_via_second_parent_is_allowed) {
    wchar_t temp_dir[MAX_PATH];
    if (!make_temp_merge_repo(temp_dir, MAX_PATH)) {
        TEST_SKIP("Could not create temporary merge repo (git not available?)");
    }
    ...
    // Merge HEAD has two parents. Side tip is parent2 (second parent of merge).
    GitCommit merge = {};
    TEST_ASSERT_TRUE(git_read_commit(&repo, repo.head_sha, &merge));
    TEST_ASSERT_TRUE(merge.parent_sha[0] != '\0');
    TEST_ASSERT_TRUE(merge.parent2_sha[0] != '\0');

    char side_tip[41];
    strcpy_s(side_tip, merge.parent2_sha);

    // Swapped-arg (push) direction: is remote (side tip) an ancestor of local (merge)?
    // Yes — side is second parent of merge. Full-graph FF allows this push.
    TEST_ASSERT_TRUE(test_is_fast_forward(&repo, side_tip, repo.head_sha));

    // Control: first parent of merge is also an ancestor
    TEST_ASSERT_TRUE(test_is_fast_forward(&repo, merge.parent_sha, repo.head_sha));

    // Control: merge is NOT an ancestor of side tip (would be non-FF if pushing side→merge base wrong way)
    TEST_ASSERT_FALSE(test_is_fast_forward(&repo, repo.head_sha, side_tip));

    // Equal SHAs are FF (already up to date)
    TEST_ASSERT_TRUE(test_is_fast_forward(&repo, repo.head_sha, repo.head_sha));
