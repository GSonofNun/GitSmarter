...
// Table of Contents
//
// 1. File Type Detection (mirrors logic in git_diff.cpp) (line 19)
// 2. Construction / Destruction (line 78)
// 3. Dirty State (line 219)
// 4. Edit Operations (line 235)
//
// </AUTO-GENERATED TOC>
...
bool TextDocument::undo() {
    // PieceTable::undo returns cursor position (size_t), not success.
    // Cursor 0 is a valid restore — never treat it as failure (FIND-059).
    if (!piece_table.can_undo()) {
        return false;
    }
    piece_table.undo();
...
bool TextDocument::redo() {
    // Same as undo: return value is cursor position, not bool.
    if (!piece_table.can_redo()) {
        return false;
    }
    piece_table.redo();
    dirty = (piece_table.undo_position() != clean_undo_pos);
